...
from apps.models import Book, BookChapters, BookCategory, BookChapterContent
import re
...
class LiteratureCrawler():
    def __init__(self):
        self.start_url = 'https://www.bqgww.cc'
        self.headers = {
    ...
    def crawling_book_category(self):
        ...
        categories = res.xpath('//div[@class="nav"]/ul/li[position()>1]')
        ...
        for category in categories:
            ...
            if response.get('code') == 0:
                ...
                self.crawling_book_info(self.start_url + books_url, category_db.cate_id, category_db.cate_name)
            else:
                print("请求失败:", response)
    ...
    def crawling_book_info(self, books_url, cate_id, cate_name):
        ...
        res = etree.HTML(html.content.decode())
        items = res.xpath('//div[@class="item"]')
        for it in items:
            try:
                book_name = it.xpath('./div[@class="image"]/a/img/@alt')[0]
                book_url = it.xpath('./div[@class="image"]/a/@href')[0]
                book_author = it.xpath('./dl/dt/span/text()')[0]
                book_category = cate_name
                self.crawling_book_content(book_name, self.start_url + book_url, book_author, book_category, cate_id, cate_name)
            except IndexError:
                pass
    ...
    def crawling_book_content(self, book_name, book_url, book_author, book_category, cate_id, cate_name):
        ...
        res = etree.HTML(html.content.decode())
        chapters = res.xpath('//div[@class="listmain"]/dl/dd/a')
        intro = res.xpath('//div[@class="intro"]/dl/dd/text()')
        book_db = Book({
            'book_name': book_name,
            'cate_id': cate_id,
            'cate_name': cate_name,
        ...
        if response.get('code') == 0:
            ...
            for i, chapter in enumerate(chapters):
                if i + 1 <= last:
                    continue
                chapter_url = self.start_url + chapter.xpath('./@href')[0]
    ...
    def crawling_book_chapter(self, chapter_url, book_id, last):
        ...
        res = etree.HTML(html)
        chapter_name = res.xpath('//div[@class="content"]/h1/text()')[0]
        ...
        if chapter_id <= last and chapter_id != -1:
            return
        #content = res.xpath('//div[@id="chaptercontent"]')[0].xpath('string(.)')
        content = res.xpath('//div[@id="chaptercontent"]/text()')
        # content = re.findall(r'<div id="chaptercontent"><!--go-->(.*?)<!--over-->\s+</div>', html, re.S)
        if len(content) > 1:
            content = content[0]
